...
const fieldFilters_1 = require("./fieldFilters");
...
const createToDoItem = (req, res) => {
    return (0, Function_1.pipe)(req.body, requestParameters_1.safeParseCreateToDoFieldsFromBody, Effect.orElseFail(() => new customErrors_1.ParameterError({
        message: "Invalid input in request body. Expected {text: string} (non-empty)",
    })), Effect.flatMap((body) => (0, todos_1.createTodoQuery)(body.text)), Effect.flatMap(todos_1.parseTodo), (getDataEffect) => (0, sendResponse_1.sendResponse)({
        getDataEffect,
        response: res,
        successStatus: 201,
    }));
...
const getAllToDoItems = (req, res) => {
    const maybeSafeArgs = {
        ...
        filters: (0, fieldFilters_1.getFilterParamsFromRequest)(req),
    ...
    return (0, Function_1.pipe)(Effect.all(maybeSafeArgs), Effect.flatMap((safeArgs) => (0, todos_1.selectAllTodosQuery)(safeArgs)), Effect.flatMap(todos_1.parseTodoArray), (getDataEffect) => (0, sendResponse_1.sendResponse)({
        getDataEffect,
        response: res,
        successStatus: 200,
    }));
...
const getToDoItem = (req, res) => {
    const maybeSafeArgs = {
        id: (0, requestParameters_1.safeParseIDRequestParameter)(req),
        definedFields: (0, definedFields_1.safeParseDefinedFields)(req.query.fields).pipe(Effect.orElseSucceed(() => ["id", "text", "updated_at"])),
    };
    return (0, Function_1.pipe)(Effect.all(maybeSafeArgs), Effect.flatMap((safeArgs) => (0, todos_1.selectTodoByIdQuery)(safeArgs)), Effect.flatMap(parseHelpers_1.checkIfNoResult), Effect.flatMap(todos_1.parseTodo), (getDataEffect) => (0, sendResponse_1.sendResponse)({
        getDataEffect,
        response: res,
        successStatus: 200,
    }));
...
const deleteToDoItem = (req, res) => {
    return (0, Function_1.pipe)((0, requestParameters_1.safeParseIDRequestParameter)(req), Effect.flatMap((id) => (0, todos_1.deleteByIdQuery)(id)), Effect.flatMap(parseHelpers_1.checkIfNoResult), (getDataEffect) => (0, sendResponse_1.sendResponse)({
        getDataEffect,
        response: res,
        successStatus: 204,
    }));
...
const updateToDoItem = (req, res) => {
    ...
    return (0, Function_1.pipe)((0, requestParameters_1.checkNoExcessFieldsForUpdate)(req.body), Effect.flatMap(() => Effect.all(maybeSafeArgs)), Effect.flatMap((safeArgs) => (0, todos_1.updateTodosQuery)(safeArgs)), Effect.flatMap(parseHelpers_1.checkIfNoResult), Effect.flatMap(todos_1.parseTodo), (getDataEffect) => (0, sendResponse_1.sendResponse)({
        getDataEffect,
        response: res,
        successStatus: 200,
    }));
